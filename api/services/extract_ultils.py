...
def get_caption(page, img_rect):
    ...
    for line in lines:
        if (
            img_rect.y1 < page.rect.height / 2
            and img_rect.y1 - 20 <= page.rect.height / 2
        ):
            caption = line.strip()
            break
    ...
    try:
        ...
        subprocess.run(
            [
                ...
                "--outdir",
                os.path.dirname(output_path),  # 输出的 PDF 文件目录
            ],
            check=True,
        )
    ...
    for filename in os.listdir(input_folder):
        if filename.endswith(".doc") or filename.endswith(".docx"):
            ...
            output_path = os.path.join(
                output_folder, filename.rsplit(".", 1)[0] + ".pdf"
            )
    ...
    try:
        ...
        for col in df.columns:
            df[col] = (
                df[col]
                .astype(str)
                .apply(lambda x: f"'{x}" if "-" in x and len(x.split("-")) == 3 else x)
            )

        # 保存为 CSV 文件，并指定格式
        df.to_csv(
            filename, index=False, encoding="utf-8-sig", quoting=csv.QUOTE_NONNUMERIC
        )
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for page_number in range(1, total_pages + 1):
            ...
            if not tables:
                ...
                tables = extract_table_with_camelot(
                    pdf_path, page_number, flavor="stream"
                )
